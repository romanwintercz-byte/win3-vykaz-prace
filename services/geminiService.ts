...
Každý denní záznam v poli 'days' obsahuje:
- 'projectEntries': pole časových záznamů pro odpracované úseky. Každý záznam v poli obsahuje 'projectId', 'hours', 'overtime' a 'notes'. 'projectId' odkazuje na 'id' z hlavního seznamu 'projects'.
- 'absenceId': odkaz na 'id' z hlavního seznamu 'absences'.
- 'absenceHours': počet hodin absence pro daný den.

Jeden den může obsahovat jak odpracované hodiny (v 'projectEntries'), tak hodiny absence ('absenceHours').

Data: ${JSON.stringify(data, null, 2)}
Ve svém shrnutí se zaměř na následující body:
1.  Celkový počet odpracovaných hodin a přesčasů za celý měsíc.
2.  Analýza rozložení práce mezi jednotlivé projekty/činnosti. Pro výpočet součtů za jednotlivé projekty musíš projít pole 'projectEntries' u každého dne a sečíst hodnoty 'hours' a 'overtime'. Použij názvy projektů ze seznamu 'projects'. Uveď, které projekty byly nejvíce časově náročné.
