...
const TimesheetStatus: React.FC<TimesheetStatusProps> = ({ workTimeFund, totalWorkHours, totalAbsenceHours }) => {
    if (workTimeFund === 0) {
        return (
    ...
    const totalReportedHours = totalWorkHours + totalAbsenceHours;
    const difference = totalReportedHours - workTimeFund;
    
    const workPercent = workTimeFund > 0 ? (totalWorkHours / workTimeFund) * 100 : 0;
    ...
    return (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 col-span-1 md:col-span-2 lg:col-span-2">
            ...
            <div className="mt-4 w-full bg-slate-200 rounded-full h-6 relative overflow-hidden" title={`Odpracováno: ${totalWorkHours.toFixed(2)}h, Absence: ${totalAbsenceHours}h`}>
                <div className="h-full flex">
                    <div style={{ width: `${Math.min(workPercent, 100)}%` }} className="bg-green-500 h-full transition-all duration-300"></div>
                    <div style={{ width: `${Math.min(absencePercent, 100 - workPercent)}%` }} className="bg-yellow-400 h-full transition-all duration-300"></div>
                     {totalPercent > 100 && (
                         <div style={{ width: `${Math.min(totalPercent - 100, 100)}%` }} className="bg-orange-500 h-full transition-all duration-300"></div>
                    )}
                </div>
                {difference < 0 && (
                    <div style={{ width: `${100 - totalPercent}%`, left: `${totalPercent}%`, background: 'repeating-linear-gradient(-45deg, #e2e8f0, #e2e8f0 5px, #fecaca 5px, #fecaca 10px)' }} className="absolute top-0 h-full transition-all duration-300"></div>
                )}
            </div>
