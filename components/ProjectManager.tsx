...
export const ProjectManager: React.FC<ProjectManagerProps> = ({ isOpen, onClose, projects, onAddProject, onUpdateProject, onArchiveProject }) => {
    ...
    const handleAddProject = (e: React.FormEvent) => {
        ...
        if (newProjectName.trim()) {
            ...
            setNewProjectName('');
            setNewProjectColor(`#${Math.floor(Math.random()*16777215).toString(16).padStart(6, '0')}`);
        }
    };
        ...
        >
            ...
            >
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-2xl font-bold text-slate-900">Správa projektů a činností</h2>
                            ...
                            <div className="flex items-center gap-2">
                               {editingProject?.id === project.id ? (
                                   ...
                                   <>
                                        ...
                                        <button onClick={() => handleStartEdit(project)} className="p-1.5 text-slate-500 hover:bg-slate-200 rounded-md" aria-label="Upravit"><PencilIcon /></button>
                                   </>
                               )}
                            </div>
                        </div>
                ...
                <form onSubmit={handleAddProject} className="mt-6 flex gap-2">
                    <input
                        ...
                        className="h-10 w-10 p-1 border border-slate-300 rounded-lg cursor-pointer"
                    />
                    <input
                        type="text"
